...
function DetailProduct() {
  ...
  return (
    <div className="flex flex-col bg-white h-screen relative">
      <div className="w-full flex justify-center md:bg-gray-200">
        <PhotoProvider className={`${!product.is_available && "grayscale"}`}>
          <PhotoView src={product.image}>
            <img
              src={product.image}
              alt="image product"
              className={`max-h-[20rem] w-full md:w-52 relative object-contain rounded-t-lg hover: cursor-zoom-in object-top ${
        ...
        <Link
          to={"/list"}
          className="absolute start-5 top-5 bg-gray-50 p-2 rounded-md text-black"
        >
      ...
      <div className=" w-full absolute bottom-0 flex items-center md:justify-center">
        <div className="md:w-1/2 flex h-12 text-white font-xs w-full md:justify-center">
          ...
          <div className="bg-green-600 w-1/2 flex items-center justify-center font-serif">
            <Link to={'/payment/' + product.username} className=" text-center text-white">Order</Link>
          </div>
        </div>
      </div>
    </div>
