...
function Menu() {
  ...
  return (
    <div className="fixed bottom-0 h-14 w-full md:w-1/2 text-xs shadow-md">
      <ul className="bg-primary h-full flex items-center gap-2 justify-around px-10 py-2 text-white">
        <Link to={"/list"} className={`flex flex-col items-center gap-1 ${location.pathname == "/list" ? activeClass : noActiveClass}`}>
          <SlHome
            className={`text-xl hover:opacity-100`}
            title="Produk"
          />
          Produk
        </Link>
        <Link to={"/cart"} className={`flex flex-col items-center gap-1 ${location.pathname == "/cart" ? activeClass : noActiveClass}`}>
          <SlBasket
            className={`text-xl hover:opacity-100`}
            title="Keranjang"
          />
        ...
        <Link to={"#"} className={`flex flex-col items-center gap-1 ${location.pathname == "/me" ? activeClass : noActiveClass}`}>
          <SlUser
            className={`text-xl hover:opacity-100`}
            title="Profil"
          />
